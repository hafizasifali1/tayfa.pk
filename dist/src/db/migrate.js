    ...
    try {
      ...
      `);
        ...
        try {
            ...
            const { roles } = await Promise.resolve().then(() => __importStar(require('./schema')));
            for (const role of defaultRoles) {
                const [existing] = await index_1.db.select().from(roles).where((0, drizzle_orm_1.eq)(roles.id, role.id));
                if (!existing) {
        ...
        try {
            ...
            const { emailTemplates } = await Promise.resolve().then(() => __importStar(require('./schema')));
          ...
          `,
            ...
            for (const template of defaultTemplates) {
                try {
                    const [existing] = await index_1.db.select().from(emailTemplates).where((0, drizzle_orm_1.eq)(emailTemplates.name, template.name));
                    if (!existing) {
                        await index_1.db.insert(emailTemplates).values({
                            ...template,
                            isActive: true,
                            createdAt: new Date(),
                            updatedAt: new Date()
