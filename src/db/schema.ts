...
export const refundRequests = isMysql 
  ? mysqlTable('refund_requests', {
      ...
      reason: mysqlLongText('reason').notNull(),
      proofImages: mysqlJson('proof_images'),
      paymentProof: mysqlLongText('payment_proof'),
      refundMethod: mysqlVarchar('refund_method', { length: 100 }),
      status: mysqlVarchar('status', { length: 50 }).default('pending'),
      adminNote: mysqlLongText('admin_note'),
      confirmationReceipt: mysqlLongText('confirmation_receipt'),
  ...
  : pgTable('refund_requests', {
      ...
      confirmationReceipt: pgText('confirmation_receipt'),
...
export const returns = isMysql
  ? mysqlTable('returns', {
      id: mysqlChar('id', { length: 36 }).primaryKey(),
      orderId: mysqlChar('order_id', { length: 36 }).notNull(),
      orderItemId: mysqlChar('order_item_id', { length: 36 }),
      userId: mysqlChar('user_id', { length: 36 }),
      reason: mysqlLongText('reason'),
      proofImages: mysqlJson('proof_images'),
      paymentProof: mysqlLongText('payment_proof'),
      returnMethod: mysqlVarchar('return_method', { length: 100 }),
      status: mysqlVarchar('status', { length: 50 }).default('requested'),
      adminNote: mysqlLongText('admin_note'),
      courierSlip: mysqlLongText('courier_slip'),
      courierId: mysqlVarchar('courier_id', { length: 255 }),
      requestedAt: mysqlTimestamp('requested_at').defaultNow(),
      createdAt: mysqlTimestamp('created_at').defaultNow(),
      updatedAt: mysqlTimestamp('updated_at').defaultNow(),
    })
  : pgTable('returns', {
      id: pgUuid('id').defaultRandom().primaryKey(),
      orderId: pgText('order_id').notNull(),
      orderItemId: pgText('order_item_id'),
      userId: pgText('user_id'),
      reason: pgText('reason'),
      proofImages: pgJsonb('proof_images'),
      paymentProof: pgText('payment_proof'),
      returnMethod: pgVarchar('return_method', { length: 100 }),
      receiptConfirmed: pgBoolean('receipt_confirmed').default(false),
      status: pgVarchar('status', { length: 50 }).default('requested'),
      adminNote: pgText('admin_note'),
      courierSlip: pgText('courier_slip'),
      courierId: pgVarchar('courier_id', { length: 255 }),
      requestedAt: pgTimestamp('requested_at').defaultNow(),
      createdAt: pgTimestamp('created_at').defaultNow(),
      updatedAt: pgTimestamp('updated_at').defaultNow(),
    });