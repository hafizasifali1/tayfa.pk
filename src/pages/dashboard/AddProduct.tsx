...
const AddProduct = () => {
  ...
  const handleSubmit = async (overrideStatus?: 'draft' | 'published' | 'archived') => {
    ...
    // For published products, we need all basic info
    if (status === 'published') {
      ...
      // For drafts, we at least need a name to identify it
      if (!formData.name) {
        setError('Please enter at least a product name to save as draft.');
        return;
      }
    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  ...
                  <input
                    ...
                    placeholder="0.00"
                  />
                  <p className="mt-1 text-[8px] text-brand-dark/40 uppercase tracking-widest">Enter price in USD. It will be converted automatically.</p>
                </div>
                <div>
