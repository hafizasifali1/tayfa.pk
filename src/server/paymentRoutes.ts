...
import { getGateway } from './payments/gatewayInterface';
...
router.put('/admin/gateways/:id', async (req, res) => {
  try {
    ...
    const [updatedGateway] = await db.select().from(paymentGateways).where(eq(paymentGateways.id, req.params.id));
...
router.post('/admin/gateways/:id/config', async (req, res) => {
  try {
    ...
    const encryptedValue = key.toLowerCase().includes('secret') || key.toLowerCase().includes('key') ? encrypt(value) : value;
...
router.get('/checkout/gateways', async (req, res) => {
  try {
    ...
    const activeGateways = await db.select().from(paymentGateways).where(eq(paymentGateways.status, true));
    
    // Filter by rules (simplified logic)
    const eligibleGateways = [];
    for (const gateway of activeGateways) {
      const rules = await db.select().from(gatewayRules).where(eq(gatewayRules.gatewayId, gateway.id));
      if (rules.length === 0) {
...
router.post('/checkout/initiate-payment', async (req, res) => {
  try {
    ...
    // Fetch config
    const configs = await db.select().from(gatewayConfigs).where(eq(gatewayConfigs.gatewayId, gateway.id));
