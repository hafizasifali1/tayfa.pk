...
// import { AlertCircle, CheckCircle2, Info, Sparkles, Share2, Globe, Eye, Image as ImageIcon, Bot, ShieldAlert } from 'lucide-react';
...
import GooglePreview from './GooglePreview';
import { GoogleGenAI, Type } from "@google/genai";
>>>>>>> f3b86c9611ad77edd989841e06907e604fc93be6
...
const SEOSettingsForm: React.FC<SEOSettingsFormProps> = ({ 
  ...
  useEffect(() => {
    // Pre-fill canonical URL if pagePath is available and canonicalUrl is empty
    if (pagePath && !metadata.canonicalUrl) {
      const origin = typeof window !== 'undefined' ? window.location.origin : '';
      // Ensure pagePath starts with / if it's not empty and doesn't have it
...
//         contents: `Generate SEO meta title (max 60 chars) and meta description (max 160 chars) for a ${entityType} named "${entityData.name || entityData.title}". 
...
=======
    ...
    try {
      const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY || '' });
      const keywordsPrompt = metadata.keywords ? ` Use these keywords if relevant: ${metadata.keywords}.` : '';
      
      const response = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: `Generate SEO meta title (max 60 chars) and meta description (max 160 chars) for a ${entityType} named "${entityData.name || entityData.title}". 
        Description: ${entityData.description || ''}.${keywordsPrompt}
        Return as JSON with "title" and "description" keys.`,
        config: {
          responseMimeType: "application/json",
          responseSchema: {
...
//             className="flex items-center space-x-2 px-4 py-2 bg-brand-dark text-white rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-brand-gold transition-all disabled:opacity-50"
//           >
...
=======
      ...
      {aiSuggestions && (
        <div className="bg-brand-gold/5 border border-brand-gold/20 rounded-2xl p-6 space-y-4 animate-in fade-in slide-in-from-top-4 duration-500">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              <button
                ...
                className="text-[10px] font-bold uppercase tracking-widest text-brand-dark/40 hover:text-brand-dark transition-colors"
              >
                Discard
              </button>
              <button
                onClick={handleGenerateAI}
                disabled={isGenerating}
                className="text-[10px] font-bold uppercase tracking-widest text-brand-gold hover:text-brand-gold-dark transition-colors flex items-center"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <p className="text-[10px] font-bold uppercase tracking-widest text-brand-dark/40">Suggested Title</p>
              <p className="text-sm font-medium text-brand-dark bg-white p-3 rounded-xl border border-brand-dark/5">{aiSuggestions.title}</p>
            </div>
            <div className="space-y-2">
              <p className="text-[10px] font-bold uppercase tracking-widest text-brand-dark/40">Suggested Description</p>
              <p className="text-xs text-brand-dark/60 bg-white p-3 rounded-xl border border-brand-dark/5 leading-relaxed">{aiSuggestions.description}</p>
            </div>
          </div>

          <button
            onClick={applySuggestions}
            className="w-full py-3 bg-brand-gold text-white rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-brand-gold-dark transition-all shadow-lg shadow-brand-gold/20 flex items-center justify-center"
          >
...
//           className="w-full px-4 py-3 border border-brand-dark/10 rounded-xl focus:ring-brand-gold focus:border-brand-gold text-sm font-mono"
//         />
//         <p className="mt-1 text-[10px] text-brand-dark/40 italic">
//           Advanced: Add schema.org markup for better rich results.
//         </p>
//       </div>
//     </div>
//   );
...
=======
  ...
  "description": "Product Description"
        ...
        <p className="mt-2 text-[10px] text-brand-dark/40 italic flex items-start">
          ...
          Advanced: Add schema.org markup (JSON-LD) to help search engines understand your content and enable rich results (like star ratings or product prices) in search.
        </p>
      </div>
    </div>
  );
